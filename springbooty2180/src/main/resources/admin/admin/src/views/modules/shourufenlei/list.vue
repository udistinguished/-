        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt"
                    ...
                    <el-form-item :label="contents.inputTitle == 1 ? '收入分类' : ''">
                        <el-input v-if="contents.inputIcon == 1 && contents.inputIconPosition == 1"
                                  prefix-icon="el-icon-search" v-model="searchForm.shourufenlei" placeholder="收入分类"
                                  clearable></el-input>
                        <el-input v-if="contents.inputIcon == 1 && contents.inputIconPosition == 2"
                                  suffix-icon="el-icon-search" v-model="searchForm.shourufenlei" placeholder="收入分类"
                                  clearable></el-input>
                        <el-input v-if="contents.inputIcon == 0" v-model="searchForm.shourufenlei" placeholder="收入分类"
                                  clearable></el-input>
                    </el-form-item>
                    <el-form-item>
                ...
                <el-row class="ad"
                    ...
                    <el-form-item>
                        <el-button
                                v-if="isAuth('shourufenlei','新增') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 1"
                                type="success"
                                icon="el-icon-plus"
                        ...
                        <el-button
                                v-if="isAuth('shourufenlei','新增') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 2"
                                type="success"
                                @click="addOrUpdateHandler()"
                        >{{ contents.btnAdAllFont == 1?'新增':'' }}<i class="el-icon-plus el-icon--right"/></el-button>
                        <el-button
                                v-if="isAuth('shourufenlei','新增') && contents.btnAdAllIcon == 0"
                                type="success"
                                @click="addOrUpdateHandler()"
                        >{{ contents.btnAdAllFont == 1?'新增':'' }}
                        </el-button>
                        <el-button
                                v-if="isAuth('shourufenlei','删除') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 1 && contents.tableSelection"
                        ...
                        <el-button
                                v-if="isAuth('shourufenlei','删除') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 2 && contents.tableSelection"
                                :disabled="dataListSelections.length <= 0"
                                type="danger"
                                @click="deleteHandler()"
                        >{{ contents.btnAdAllFont == 1?'删除':'' }}<i class="el-icon-delete el-icon--right"/></el-button>
                        <el-button
                                v-if="isAuth('shourufenlei','删除') && contents.btnAdAllIcon == 0 && contents.tableSelection"
                                :disabled="dataListSelections.length <= 0"
                                type="danger"
                                @click="deleteHandler()"
                        >{{ contents.btnAdAllFont == 1?'删除':'' }}
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            <el-button
                                    v-if="isAuth('shourufenlei','查看') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1"
                                    type="success" icon="el-icon-tickets" size="mini"
                                    @click="addOrUpdateHandler(scope.row.id,'info')">{{ contents.tableBtnFont ==
                                1?'详情':'' }}
                            </el-button>
                            <el-button
                                    v-if="isAuth('shourufenlei','查看') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2"
                            ...
                            <el-button v-if="isAuth('shourufenlei','查看') && contents.tableBtnIcon == 0" type="success"
                                       size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">{{
                                contents.tableBtnFont == 1?'详情':'' }}
                            </el-button>
                            <el-button
                                    v-if="isAuth('shourufenlei','修改') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1"
                                    type="primary" icon="el-icon-edit" size="mini"
                                    @click="addOrUpdateHandler(scope.row.id)">{{ contents.tableBtnFont == 1?'修改':'' }}
                            </el-button>
                            <el-button
                                    v-if="isAuth('shourufenlei','修改') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2"
                            ...
                            <el-button v-if="isAuth('shourufenlei','修改') && contents.tableBtnIcon == 0" type="primary"
                            ...
                            <el-button
                                    v-if="isAuth('shourufenlei','删除') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1"
                                    type="danger" icon="el-icon-delete" size="mini"
                                    @click="deleteHandler(scope.row.id)">{{ contents.tableBtnFont == 1?'删除':'' }}
                            </el-button>
                            <el-button
                                    v-if="isAuth('shourufenlei','删除') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2"
                                    type="danger" size="mini" @click="deleteHandler(scope.row.id)">{{
                                contents.tableBtnFont == 1?'删除':'' }}<i class="el-icon-delete el-icon--right"/>
                            </el-button>
                            <el-button v-if="isAuth('shourufenlei','删除') && contents.tableBtnIcon == 0" type="danger"
    ...
    export default {
        ...
        methods: {
            ...
            getDataList() {
                ...
                if (this.searchForm.shourufenlei != '' && this.searchForm.shourufenlei != undefined) {
                    params['shourufenlei'] = '%' + this.searchForm.shourufenlei + '%'
                }
                this.$http({
                    url: "shourufenlei/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if (data && data.code === 0) {
