<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="储蓄类型" prop="chuxuleixing">
          <el-select v-model="ruleForm.chuxuleixing" placeholder="请选择储蓄类型">
            <el-option
                v-for="(item,index) in chuxuleixingOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="储蓄类型" prop="chuxuleixing">
	      <el-input v-model="ruleForm.chuxuleixing"
                placeholder="储蓄类型" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="银行账户" prop="yinxingzhanghu">
          <el-input v-model="ruleForm.yinxingzhanghu" 
              placeholder="银行账户" clearable  :readonly="ro.yinxingzhanghu"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="银行账户" prop="yinxingzhanghu">
              <el-input v-model="ruleForm.yinxingzhanghu" 
                placeholder="银行账户" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.yonghuxingming = true;
          ...
          if(o=='chuxuleixing'){
            this.ruleForm.chuxuleixing = obj[o];
	    this.ro.chuxuleixing = true;
            continue;
          }
	    ...
	    this.ro.yinxingmingcheng = true;
          ...
          if(o=='yinxingzhanghu'){
            this.ruleForm.yinxingzhanghu = obj[o];
	    this.ro.yinxingzhanghu = true;
            continue;
          }
          if(o=='jine'){
      ...
      this.$http({
		...
		if(json.yonghuxingming!=''&&json.yonghuxingming){
          ...
          this.$message.error(data.msg);
            ...
            this.chuxuleixingOptions = "活期,定期".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `yinxingchuxu/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `yinxingchuxu/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.yinxingchuxuCrossAddOrUpdateFlag = false;
                  this.parent.search();
